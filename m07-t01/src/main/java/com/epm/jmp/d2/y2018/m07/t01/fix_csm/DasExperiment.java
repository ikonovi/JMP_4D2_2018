package com.epm.jmp.d2.y2018.m07.t01.fix_csm;
...
public class DasExperiment {
    ...
    private Map<Integer, Integer> mapNotSyncked = new HashMap<Integer, Integer>(); // Original, that is not thread-safe.
    private Map<Integer, Integer> map = Collections.synchronizedMap(mapNotSyncked); // FIX-part1
    ...
    Runnable runSummingValues = new Runnable() {
        ...
        public void run() {
            ...
            while (++loop <= SUMMING_LOOPS_COUNT) {
                synchronized (map) { // FIX-part2: "manually synchronize on the returned map when iterating over any of
